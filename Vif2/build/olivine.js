...
(() => {
    ...
    async function loadEntry(target) {
        if (!('entry' in target)) {
            ...
            window.$__pluginPipeline__$ = createPluginPipeline(target)
            window.__slModuleImport__ = src => externImportSlModule(src, $__pluginPipeline__$, target)
            ...
            const newEntryFile = $__pluginPipeline__$(entryFile)
    ...
    function loadBuildAPI(target) {
        window.Build = {}
        Build.include = async function(path) {
            return await externImportSlModule(path, $__pluginPipeline__$, target)
        }
    }
