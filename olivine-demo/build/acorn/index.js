...
Object.defineProperty(jsxm.exports, "tokTypes", {
    get: function get_tokTypes() {
        return getJsxTokens(window._Acorn).tokTypes;
    },
    configurable: true,
    enumerable: true
});
...
function plugin(options, Parser) {
    const acorn = Parser.acorn || window._Acorn;
...
window._Acorn.parseExpressionAt = parseExpressionAt
...
window._Acorn.tokenizer = tokenizer
...
window._Walk.ancestor = ancestor
...
// nodes).
window._Walk.recursive = recursive
...
window._Walk.fullAncestor = fullAncestor
...
window._Walk.findNodeAt = findNodeAt
...
window._Walk.findNodeAround = findNodeAround
...
window._Walk.findNodeAfter = findNodeAfter
...
window._Walk.findNodeBefore = findNodeBefore
...
base.JSXElement = function (node, state, c) {
    for (let i = 0; i < node.openingElement.attributes.length; i++) {
        c(node.openingElement.attributes[i], state);
    }
    for (let i = 0; i < node.children.length; i++) {
        c(node.children[i], state);
    }
};
...
base.JSXIdentifier = function (node, state, c) {};

base.JSXExpressionContainer = function (node, state, c) {
    c(node.expression, state);
};

base.JSXText = function () {};

base.JSXFragment = function (node, state, c) {
    for (let i = 0; i < node.children.length; i++) {
        c(node.children[i], state);
    }
};

///  begins loading assets! injected by olivine for the use of createLoaderQueue
window.$__lqnext__$()
